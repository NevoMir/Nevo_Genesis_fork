...
class MPMSolver(Solver):
    ...
    def __init__(self, scene: "Scene", sim: "Simulator", options: "MPMOptions"):
        ...
        # options
        self._grid_density = options.grid_density
        self._particle_size = float(options.particle_size)
        self._upper_bound = np.array(options.upper_bound)
        self._lower_bound = np.array(options.lower_bound)
        self._leaf_block_size = options.leaf_block_size
        self._use_sparse_grid = options.use_sparse_grid
        ...
        # NEW: dynamic particle size options
        self._size_growth_rate = float(getattr(options, "particle_size_growth_rate", 0.0) or 0.0)
        self._size_growth_mode = getattr(options, "particle_size_growth_mode", "preserve_density")
        self._size_min = float(getattr(options, "particle_size_min", 1e-6))
        self._size_max = getattr(options, "particle_size_max", None)

        self._n_vvert_supports = self.scene.vis_options.n_support_neighbors

        # NOTE: the magnitude of `_p_vol` doesn't affect MPM simulation itself, but it's used to compute particle mass,
        # the scale of which matters in coupling. `_p_vol_scale` is used to avoid potential numerical instability,
        # as the actual `_p_vol` is very small. We need to account for this scale when handling coupling.
        self._p_vol_real = float(self._particle_size**3)
        self._p_vol_scale = float(1e3)
        self._p_vol = self._p_vol_real * self._p_vol_scale
    ...
    # ------------------------------------------------------------------------------------
    # --------------------------- dynamic particle size helpers --------------------------
    # ------------------------------------------------------------------------------------

    @ti.kernel
    def _kernel_rescale_particle_masses(self, scale: gs.ti.f32):
        # Rescales stored per-particle mass by a scalar factor.
        for i_p in range(self._n_particles):
            self.particles_info[i_p].mass *= scale

    def set_particle_size(self, new_size: float, preserve_density: Optional[bool] = None):
        """
        Change the global particle size (diameter) used by this MPM solver.
        If preserve_density is True (default), per-particle masses are scaled with volume
        so density stays constant. If False, masses are left unchanged (density falls when size grows).
        """
        if new_size <= 0.0:
            gs.raise_exception("`new_size` must be > 0.")
        # clamp to user-provided bounds if any
        if new_size < self._size_min:
            new_size = self._size_min
        if (self._size_max is not None) and (new_size > self._size_max):
            new_size = self._size_max

        old_size = self._particle_size
        if abs(new_size - old_size) < 1e-15:
            return

        # update solver-level size and derived volumes
        old_real_vol = self._p_vol_real
        self._particle_size = float(new_size)
        self._p_vol_real = float(self._particle_size ** 3)
        vol_scale_real = self._p_vol_real / old_real_vol
        self._p_vol = self._p_vol_real * self._p_vol_scale  # stress scale uses _p_vol

        # rescale masses if we preserve density
        if preserve_density is None:
            preserve_density = (self._size_growth_mode == "preserve_density")

        if preserve_density:
            # multiply stored mass by volume scale so rho stays constant
            self._kernel_rescale_particle_masses(gs.np_float(vol_scale_real))

        # sanity hint if size gets too large relative to grid spacing
        if self._particle_size > 0.5 * self._dx:
            gs.logger.warning(
                f"Particle size ({self._particle_size:.4g}) > 0.5*dx ({0.5*self._dx:.4g}). "
                "This may introduce artifacts; consider increasing grid_density."
            )

    def _maybe_apply_growth(self):
        # multiplicative growth for diameter: size <- size * (1 + rate * dt)
        if self._size_growth_rate != 0.0:
            factor = 1.0 + self._size_growth_rate * self.substep_dt
            # minimal guard
            if factor <= 0.0:
                return
            self.set_particle_size(self._particle_size * factor)
    ...
    def substep_pre_coupling(self, f):
        # NEW: grow particle size gradually (if enabled)
        self._maybe_apply_growth()
